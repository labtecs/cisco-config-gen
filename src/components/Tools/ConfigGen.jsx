...
import { CheckSquare, Zap, Shield, FastForward, Phone, Power, Filter, Activity, GitCommit } from 'lucide-react';
            ...
            <main className={mainContainerClass}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-[1000px] relative">
                            {/* HEADER AREA */}
                                    ...
                                    <div className="flex items-center gap-4 self-end md:self-auto">
                                        ...
                                        {logic.viewMode === 'multi' && (
                                            <div className="flex items-center bg-slate-100 p-1 rounded-lg border border-slate-200 shadow-sm">
                                                <button
                                                    onClick={() => logic.setShowChannelGroupColumn(!logic.showChannelGroupColumn)}
                                                    className={`px-2 py-1 text-[10px] font-bold uppercase rounded transition-colors flex items-center gap-1 ${logic.showChannelGroupColumn ? 'bg-white text-purple-600 shadow-sm border border-slate-200' : 'text-slate-400 hover:text-slate-600'}`}
                                                    title="Show Port-Channel"
                                                >
                                                    <GitCommit size={10} className={logic.showChannelGroupColumn ? "fill-purple-500" : ""} /> P-CHANNEL
                                                </button>
                                ...
                                {logic.viewMode === 'multi' && (
                                    <div className="space-y-3">
                                            ...
                                            <div className="flex items-start gap-2 text-xs animate-in fade-in">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {logic.availableVlans.map(vlanObj => {
                                                        ...
                                                        if (vlanObj.status === 'unused') styleClass = "bg-orange-200 hover:bg-orange-300 text-orange-900 border-orange-400 font-bold shadow-sm";
                                                        else if (vlanObj.status === 'manual') styleClass = "bg-indigo-100 hover:bg-indigo-200 text-indigo-800 border-indigo-300";
                                                        else if (vlanObj.status === 'default') styleClass = "bg-slate-100 text-slate-400 border-slate-200 cursor-default";
                                                        const tooltip = vlanObj.name ? `VLAN ${vlanObj.id}: ${vlanObj.name}` : vlanObj.status === 'default' ? "VLAN 1 (Default)" : title;
                                                        return (
                                                            <button key={vlanObj.id} onClick={() => logic.selectPortsByVlan(vlanObj.id)} className={`border px-2 py-0.5 rounded-full transition-colors font-mono cursor-pointer flex items-center gap-1 ${styleClass}`} title={tooltip}>
                                                                {vlanObj.id}
                                                            </button>
                                                        );
                                                    })}
                                        ...
                                        <div className="flex items-center gap-4 pt-2 border-t border-slate-100 text-[10px] text-slate-500 font-medium">
                                            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-slate-200 border border-slate-300"></div><span>Genutzt (Config)</span></div>
                                            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-blue-200 border border-blue-200"></div><span>Neu / Manuell</span></div>
                                            <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-orange-200 border border-orange-400"></div><span className="text-orange-400 font-bold">Ungenutzt (Running-Config)</span></div>
                                        </div>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="flex-1 overflow-auto bg-slate-50/50">
                                {logic.viewMode === 'multi' && (
                                    <MultiPortEditor
                                        ...
                                        showChannelGroupColumn={logic.showChannelGroupColumn}
                                ...
                                {logic.viewMode === 'single' && (
                                    <div className="p-4">
                                        <SinglePortEditor
                                            ...
                                            portChannels={logic.portChannels}
                                            updatePortChannel={logic.updatePortChannel}
                                        />
                                    </div>
                                )}
                            </div>
                        ...
                        {/* Port-Channel Editor Box */}
                        {logic.viewMode === 'multi' && logic.portChannels.length > 0 && (
                             <PortChannelEditor
                                portChannels={logic.portChannels}
                                updatePortChannel={logic.updatePortChannel}
                            />
